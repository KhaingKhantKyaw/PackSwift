(() => {
  ...
  root.innerHTML = `
    <section id="concierge-panel" class="concierge-panel" role="dialog" aria-label="PackSwift Concierge" hidden>
      ...
      <form class="concierge-form">
        <label class="sr-only" for="concierge-input">Message PackSwift Concierge</label>
        <textarea id="concierge-input" rows="1" maxlength="1200" placeholder="Ask about your trip…" required></textarea>
  ...
  function storePendingAiTrip(recommendation, mode) {
    try {
      sessionStorage.setItem(pendingAiTripKey, JSON.stringify({
        recommendation,
        mode,
